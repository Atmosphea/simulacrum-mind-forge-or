...
const Header = () => {
  ...
  return (
    <header
      className="fixed top-0 left-0 right-0 z-50 glass-card backdrop-blur-xl border-b border-white/20"
          ...
          <div className="flex items-center space-x-4">
            {user ? (
              ...
              <>
                <Button
                  variant="outline"
                  className="hidden sm:inline-flex text-white border-white/30 hover:bg-white/10 text-sm"
                  onClick={() => setShowAuthModal(true)}
                >
                  Sign In
                </Button>
                <Button onClick={() => setShowAuthModal(true)} className="gradient-cta text-white text-sm">
                  Start Simulation
                </Button>
              </>
            )}
